...
def feat_extraction_missing(ls):
    ls_no_missing = list()
    for i in ls:
        if not pd.isna(i):
            ls_no_missing.append(i)

    return ls_no_missing


# person-level features
def par_feature_extraction(df):
    par_keep_days = 3
    dfs = []
    df_groups = df.groupby('participantID')
    df_group_names = df_groups.size().index
    ...
    for df_group_name in df_group_names:
        df_par = df_groups.get_group(df_group_name)
        if len(df_par) < par_keep_days:
            continue
            # dfs.append(df_par_diff)
            # print('Warning: empty')
        else:
            participantID = df_group_name
            ls_par_keep.append(participantID)
            df_par_col_names = df_par.columns.tolist()
            # df_par_features_mean = df_par.mean(axis = 0)
            # df_par_features_max = df_par.max(axis=0)
            # df_par_features_min = df_par.min(axis=0)
            # df_par_features_std = df_par.std(axis=0)

            df_par_list = list()
            df_par_col_names_new = list()
            df_par_col_names.remove('participantID')
            for col_name in df_par_col_names:
                ls_no_missing = feat_extraction_missing(df_par[col_name].tolist())
                if len(ls_no_missing) > 2:
                    if col_name not in pers_names:
                        df_par_list.append(np.mean(ls_no_missing))
                        df_par_col_names_new.append(col_name + '_mean')
                        df_par_list.append(np.max(ls_no_missing))
                        df_par_col_names_new.append(col_name + '_max')
                        df_par_list.append(np.min(ls_no_missing))
                        df_par_col_names_new.append(col_name + '_min')
                        df_par_list.append(np.std(ls_no_missing))
                        df_par_col_names_new.append(col_name + '_std')
                    else:
                        df_par_list.append(np.mean(ls_no_missing))
                        df_par_col_names_new.append(col_name)

                else:
                    df_par_list.append(np.nan)
                    df_par_col_names_new.append(col_name + '_mean')
                    df_par_list.append(np.nan)
                    df_par_col_names_new.append(col_name + '_max')
                    df_par_list.append(np.nan)
            ...
            df_par_df = pd.DataFrame([df_par_list], columns = df_par_col_names_new)
#             df_par_df['participantID'] = participantID
            dfs.append(df_par_df)

    df_par_output = pd.concat(dfs)
    df_par_output.reset_index(drop=True, inplace=True)
    # ls_ids = df['participantID'].tolist()
    df_par_output.index = ls_par_keep
...
def feature_extraction(df):
    ...
    df_par = par_feature_extraction(df)
